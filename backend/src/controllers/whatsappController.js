...
async function getInstanceStatus(req, res) {
  try {
    ...
    const instancesList = Array.isArray(instResp.data) ? instResp.data : [];
    const instance = instancesList.find(i => i.name === evo.instanceName);
    if (!instance) {
...
async function createInstance(req, res) {
  try {
    ...
    const instancesList = Array.isArray(instResp.data) ? instResp.data : [];
    const exists = instancesList.find(i => i.name === evo.instanceName);
    if (exists) {
